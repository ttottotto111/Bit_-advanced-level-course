...
LRESULT CALLBACK WndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_DRAWITEM:
		...
		wsprintf(str,"%s",people[dis->itemID].name);
		...
		TextOut(dis->hDC,dis->rcItem.left, dis->rcItem.top+2,str,lstrlen(str));

		SetBkMode(dis->hDC,TRANSPARENT);
		wsprintf(str,"%s",people[dis->itemID].tel);
		SetTextColor(dis->hDC,RGB(255,0,0));
		TextOut(dis->hDC,dis->rcItem.left+150, dis->rcItem.top+2,str,lstrlen(str));

		wsprintf(str,"%s",people[dis->itemID].addr);
		SetTextColor(dis->hDC,RGB(0,128,0));
		TextOut(dis->hDC,dis->rcItem.left+300, dis->rcItem.top+2,str,lstrlen(str));
		return 0;
	case WM_DESTROY:
		PostQuitMessage(0);
		return 0;
